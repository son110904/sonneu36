...
SELECT SINHVIEN.MALOP,
 ...
 COUNT(*) AS SOLUONGSV
 FROM SINHVIEN GROUP BY SINHVIEN.MALOP
...
BEGIN
    SELECT
        (SELECT TENKHOA FROM DMKHOA WHERE MAKHOA = DMLOP.MAKHOA) AS TENKHOA,
        COUNT(*) AS SL_THEOKHOA,
        (SELECT TENKHUVUC FROM DMHE WHERE MAKV = DMHE.MAKV) AS TENKHUVUC,
        COUNT(*) AS SL_THEOKHUVUC
    FROM SINHVIEN, DMLOP, DMHE
    WHERE SINHVIEN.MALOP = DMLOP.MALOP AND DMLOP.MAHE = DMHE.MAHE
    GROUP BY DMLOP.MAKHOA, DMHE.MAKV
END
